data = readlines("a07.txt").split(" ").replace(["J" "T" "Q" "K" "A"],["1" "W" "X" "Y" "Z"]); % change characters to make it sort in asci T Q K A = W X Y Z and J = 1
...
day7puzzle2result = sum(data(idx,2).double() .* (1:height(data))') % sum product of bid and rank