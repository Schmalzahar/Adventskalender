...
function cost = minCostToFinish(room, groom)
    % Returns the minimal cost to the finish position    
    pos = find(room > 0);
    cost = 0;
    part = 2;
    for i=1:height(pos)
...
%         end
        ...
        if any(start == rooms)
            ...
            % case 2: move from room to final correct room
            ...
            % case 3: move from hallway into the final correct room
            ...
            %lower_bool = isCorrect(room,ops+1);
            lower_bool = true;
            [~,below] = getAboveBelow(rooms, ops);
%             [px,~] = find(rooms_reshaped == ops);
            for j=1:length(below)
                lower_bool = lower_bool && isCorrect(room,below(j));
            end
            if ~lower_bool
